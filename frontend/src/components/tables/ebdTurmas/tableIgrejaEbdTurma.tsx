...
function IgrejaEbdTurmaFind() {
    ...
    useEffect(() => {
        ...
        // Verifique se o storedIgrejaId é um valor válido
        if (storedIgrejaId && storedIgrejaId !== 'null') {
          const idIgrejaNumber = Number(storedIgrejaId); // Converte para número
          fetchEbdAndTurmasByIgreja(idIgrejaNumber)
            .then(response => {
              if (response.data && Array.isArray(response.data.turmas)) {
                setIgrejaEbdTurmas(response.data); // Carrega os dados filtrados para essa igreja
              } else {
          ...
          // Caso não haja igrejaId, carregar todas as turmas
          fetchEbdAndTurmasByIgreja(Number(id))
            .then(response => {
              if (response.data && Array.isArray(response.data.turmas)) {
                setIgrejaEbdTurmas(response.data); // Carrega todas as turmas
              } else {
                console.error("Resposta da API não contém um array de turmas", response.data);
              }
            })
    ...
    const filteredTurmas = igrejaEbdTurmas?.turmas.filter((turma) => 
      turma.nomeTurma.toLowerCase().includes(lowerSearch)) || 
    [];
