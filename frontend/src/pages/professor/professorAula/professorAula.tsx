...
import { ProfessorAula } from "../../../types/professorAulas";
import { fetchProfessorAulas, fetchProfessorAulasPorIgreja } from "../../../utils/api";
import { Button, Pagination } from "react-bootstrap";
import { Link } from "react-router-dom";
import { BsFillPlusCircleFill, BsPencil } from "react-icons/bs";
import Header from "../../../components/header/header";

function ProfessorAndAulas(){

    const [professorAulas, setProfessorAulas] = useState<ProfessorAula[]>([]);
    ...
    useEffect(() => {
        ...
        if (igrejaId && !isNaN(Number(igrejaId))){
          fetchProfessorAulasPorIgreja(Number(igrejaId))
            .then(response => setProfessorAulas(response.data))
            .catch(error => console.log(error));
          } else {
          fetchProfessorAulas().then(response => setProfessorAulas(response.data))
          .catch(error => console.log(error))
          }
      }, []);
    ...
    const filteredProfessorAulas = professorAulas.filter((professor) => professor.
      nomeProfessor.toLowerCase().includes(lowerSearch));
  
    const indexOfLastItem = currentPage * itemsPerPage;
    const indexOfFirstItem = indexOfLastItem - itemsPerPage;
    const currentProfessorAulas = filteredProfessorAulas.slice(
      indexOfFirstItem,
      indexOfLastItem
    );    
        ...
        <>
            ...
            <div className="container mx-auto px-4 mt-6 w-screen h-screen">
                <div className="flex justify-between items-center">  
                  <h2 className="text-2xl font-bold mb-4 mt-4">Lista Professor e Aulas</h2>
                  {(!igrejaId || igrejaId === 'null') && (
                    <a href="/professorEaulas/new/0" className="flex items-center justify-center w-16 h-10 bg-blue-500 rounded-full ml-48" title="NOVO">
                ...
                {/* Tabela */}
                  <div className="overflow-x-auto bg-white rounded-lg shadow overflow-y-auto relative">
                      <table className="border-collapse table-auto w-full whitespace-no-wrap bg-white table-striped relative">
                          <thead>
                              <tr className="text-left">
                                  ...
                                  <th className="py-2 px-3 sticky top-0 border-b border-gray-200 bg-gray-100">Lição</th>
                          ...
                          <tbody>
                              {currentProfessorAulas.map((item, index) => (
                                  <tr key={item.id} className="border-b border-gray-200">
                                      ...
                                      {(!igrejaId || igrejaId === 'null') && (<td>
                                          <div>
                                              <Link to={`/professorEaulas/${item.id}`}>
                      ...
                      />
                      {[...Array(Math.ceil(filteredProfessorAulas.length / itemsPerPage))].map(
                      (_, index) => (
                          <Pagination.Item
                          key={index}
                      ...
                      <Pagination.Next
                      onClick={() => handlePageChange(currentPage + 1)}
                      disabled={currentPage === Math.ceil(filteredProfessorAulas.length / itemsPerPage)}
                      />
                </Pagination>
                  </div>
...
export default ProfessorAndAulas;